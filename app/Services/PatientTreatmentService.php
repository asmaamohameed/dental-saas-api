...
class PatientTreatmentService
{
    ...
    public function createFromTemplate(array $data, string $userId): PatientTreatment
    {
        return DB::transaction(function () use ($data, $userId) {
            ...
            if ($templateVisitCount > 0) {
                /** @var TreatmentTemplateVisit $templateVisit */
                foreach ($template->visits as $templateVisit) {
                    ...
                    /** @var TreatmentTemplateComponent $templateComponent */
    ...
    public function getInvoiceSummary(PatientTreatment $treatment): array
    {
        ...
        $visitsData = [];
        /** @var Collection<int, PatientTreatmentVisit> $treatmentVisits */
        $treatmentVisits = $treatment->visits;
        foreach ($treatmentVisits as $visit) {
            /** @var PatientTreatmentVisit $visit */
        ...
        /** @var array<int, array<string, mixed>> $visitsData */
        $visits = collect($visitsData);
    ...
    public function recomputeTreatmentStatus(PatientTreatment $treatment, string $userId): void
    {
        ...
        if ($cancelledVisits === $totalVisits) {
            ...
            $currentStatusValue = $treatment->status->value;
            if (in_array($currentStatusValue, [PatientTreatmentStatus::IN_PROGRESS->value, PatientTreatmentStatus::COMPLETED->value], true)) {
                $treatment->status = PatientTreatmentStatus::PLANNED;
                $treatment->completed_at = null;
            }
        } elseif ($pendingVisits > 0) {
    ...
    public function nextForPatient(string $patientId): ?PatientTreatment
    {
        ...
        $sorted = $nonCompleted->sortBy(function (PatientTreatment $treatment) {
            /** @var PatientTreatmentVisit|null $currentVisit */
            $currentVisit = $treatment->visits->first(function (PatientTreatmentVisit $v) {
                $s = $v->status->value;

                return $s !== PatientTreatmentVisitStatus::COMPLETED->value;
            });
            ...
            if ($currentVisit->scheduled_date) {
                return strtotime((string) $currentVisit->scheduled_date);
            }

            // Fallback: priority score
            $priorityValue = $treatment->priority->value;
            $pScore = match ($priorityValue) {
                'high' => 10,
                'medium' => 20,
                'low' => 30,
                default => 40,
    ...
    private function syncToothRecord(PatientTreatment $treatment, string $userId, ?string $condition = null): void
    {
        ...
        $treatmentStatusStr = $treatment->status->value;
        ...
        $latestConditionStr = $latest?->condition !== null ? (string) $latest->condition : null;
        $resolvedCondition = $condition ?? $latestConditionStr ?? ToothCondition::DECAYED->value;
    ...
    private function deductVisitComponents(PatientTreatmentVisit $visit): array
    {
        ...
        foreach ($visitComponents as $ptc) {
            /** @var PatientTreatmentComponent $ptc */
            if (! $ptc->component_id) {
                continue;
            }
